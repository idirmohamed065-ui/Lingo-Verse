    ...
    <div className="text-sm leading-7 break-words">
      <ReactMarkdown
        ...
        components={{
          ...
          h3: ({ children }) => (
            <h3 className="font-bold mt-3 mb-2">{children}</h3>
          ),
          p: ({ children }) => (
          ...
          code: ({ inline, children, ...props }) =>
            inline ? (
              <code
                className="px-1.5 py-0.5 rounded bg-gray-200 text-sm"
                {...props}
              >
                {children}
              </code>
            ) : (
              <pre className="overflow-x-auto rounded-lg bg-gray-900 text-white p-3 my-3">
...
export default function AIChat() {
  ...
  const createSession = useMutation(
    (data) =>
      api.post('/ai-tutor/sessions', data).then((r) => r.data.data),
    {
      onSuccess: (data) => {
      ...
      onError: (error) => {
        toast.error(
          error?.response?.data?.message ||
            'Could not start the AI session.'
        );
      }
    }
  );
  ...
  const sendMessage = useMutation(
    (data) =>
      api
        .post(`/ai-tutor/sessions/${activeSession.id}/message`, data)
        .then((r) => r.data.data),
    {
      onSuccess: (data) => {
        setMessages((prev) => [
      ...
      onError: (error) => {
        toast.error(
          error?.response?.data?.message ||
            'The AI could not process your message.'
        );
      }
    }
  );
  ...
  const handleSend = () => {
    ...
    if (!trimmedMessage || sendMessage.isLoading || !activeSession) {
      return;
    }
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <p className="text-gray-600 mb-8">
          Practice with our AI language tutor
        </p>
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 h-[calc(100vh-4rem)] flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-gray-600">
            {languages.find(
              (l) => l.id === activeSession.language_id
            )?.name}{' '}
